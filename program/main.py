...
#记录csv
leader_pose = {"leader_x": P_l_traj_interpolated[0, :num_frames_interpolated],
...
# 使用参数方程计算球面上的点
x0_a = obstacles[0][0] + obstacles[0][-1] * np.outer(np.cos(u), np.sin(v))  # x = x0 + r * cos(θ) * sin(φ)
y0_a = obstacles[0][1] + obstacles[0][-1] * np.outer(np.sin(u), np.sin(v))  # y = y0 + r * sin(θ) * sin(φ)
z0_a = obstacles[0][2] + obstacles[0][-1] * np.outer(np.ones(np.size(u)), np.cos(v))  # z = z0 + r * cos(φ)

x1_a = obstacles[1][0] + obstacles[1][-1] * np.outer(np.cos(u), np.sin(v))  # x = x0 + r * cos(θ) * sin(φ)
y1_a = obstacles[1][1] + obstacles[1][-1] * np.outer(np.sin(u), np.sin(v))  # y = y0 + r * sin(θ) * sin(φ)
z1_a = obstacles[1][2] + obstacles[1][-1] * np.outer(np.ones(np.size(u)), np.cos(v))  # z = z0 + r * cos(φ)
