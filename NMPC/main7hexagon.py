...
time_start = time.time()
P_f6_traj, step, vel, dis_to_goal = NMPCFollower(P_f6_start, P_l_goal + d6,
                                                 P_f2_traj, d6, P_f5_traj,
                                                 obstacles_new, NEIGHBOUR_SAFE,
                                                 OBS_SAFE, z_limits)
time_step_data.append(["{:3f}".format(time.time() - time_start), step])
dis_to_goal_list.append(dis_to_goal)
...
leader_pose = {
    ...
    "follow5_x": P_f5_traj_interpolated[0, :num_frames_interpolated],
    "follow5_y": P_f5_traj_interpolated[1, :num_frames_interpolated],
    "follow5_z": P_f5_traj_interpolated[2, :num_frames_interpolated],
    "follow6_x": P_f6_traj_interpolated[0, :num_frames_interpolated],
    "follow6_y": P_f6_traj_interpolated[1, :num_frames_interpolated],
    "follow6_z": P_f6_traj_interpolated[2, :num_frames_interpolated],
}
...
dis_to_goal_list.insert(0, [
    "leader", "follow1", "follow2", "follow3", "follow4", "follow5", "follow6"
])
...
f5_line, = ax.plot([], [], [], label="Follower5 Trajectory", color='pink')
f6_line, = ax.plot([], [], [], label="Follower6 Trajectory", color='purple')
...
def update_smooth(frame):
    ...
    f5_line.set_data(P_f5_traj_interpolated[0, :frame],
                     P_f5_traj_interpolated[1, :frame])
    f5_line.set_3d_properties(P_f5_traj_interpolated[2, :frame])

    f6_line.set_data(P_f6_traj_interpolated[0, :frame],
                     P_f6_traj_interpolated[1, :frame])
    f6_line.set_3d_properties(P_f6_traj_interpolated[2, :frame])
