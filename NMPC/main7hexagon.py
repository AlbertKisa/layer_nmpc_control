...
d2 = np.array([x[1], y[1], 0.0])
d3 = np.array([x[2], y[2], 0.0])
d4 = np.array([x[3], y[3], 0.0])
d5 = np.array([x[4], y[4], 0.0])
d6 = np.array([x[5], y[5], 0.0])
...
time_start = time.time()
P_f3_traj, step = NMPCFollower(P_f3_start, P_l_goal + d3, P_l_traj, d3,
                               np.empty((3, 0)), obstacles_new, NEIGHBOUR_SAFE,
                               OBS_SAFE)
time_step_data.append(["{:3f}".format(time.time() - time_start), step])
...
time_start = time.time()
P_f4_traj, step = NMPCFollower(P_f4_start, P_l_goal + d4, P_l_traj, d4,
                               P_f3_traj, obstacles_new, NEIGHBOUR_SAFE,
                               OBS_SAFE)
time_step_data.append(["{:3f}".format(time.time() - time_start), step])
...
time_start = time.time()
P_f5_traj, step = NMPCFollower(P_f5_start, P_l_goal + d5, P_l_traj, d5,
                               np.empty((3, 0)), obstacles_new, NEIGHBOUR_SAFE,
                               OBS_SAFE)
time_step_data.append(["{:3f}".format(time.time() - time_start), step])

# 計算Follower6的轨迹
print(f"P_f6_start:{P_f6_start} P_f6_goal:{P_l_goal + d6}")
time_start = time.time()
P_f6_traj, step = NMPCFollower(P_f6_start, P_l_goal + d6, P_f2_traj, d6,
                               P_f5_traj, obstacles_new, NEIGHBOUR_SAFE,
                               OBS_SAFE)
time_step_data.append(["{:3f}".format(time.time() - time_start), step])
...
P_f5_end = P_f5_traj[:, -1]
P_f6_end = P_f6_traj[:, -1]
...
P_f4_traj_interpolated = interpolate_trajectory(P_f4_traj,
                                                num_frames_interpolated)
P_f5_traj_interpolated = interpolate_trajectory(P_f5_traj,
                                                num_frames_interpolated)
P_f6_traj_interpolated = interpolate_trajectory(P_f6_traj,
                                                num_frames_interpolated)
...
leader_pose = {
    ...
    "follow2_1_x": P_f5_traj_interpolated[0, :num_frames_interpolated],
    "follow2_1_y": P_f5_traj_interpolated[1, :num_frames_interpolated],
    "follow2_1_z": P_f5_traj_interpolated[2, :num_frames_interpolated],
    "follow2_2_x": P_f6_traj_interpolated[0, :num_frames_interpolated],
    "follow2_2_y": P_f6_traj_interpolated[1, :num_frames_interpolated],
    "follow2_2_z": P_f6_traj_interpolated[2, :num_frames_interpolated],
}
...
start_end_data = [[
    ...
    P_l_traj_interpolated[2, num_frames_interpolated - 1]
                  ...
                  [
                      P_f5_start[0], P_f5_start[1], P_f5_start[2],
                      P_f5_traj_interpolated[0, num_frames_interpolated - 1],
                      P_f5_traj_interpolated[1, num_frames_interpolated - 1],
                      P_f5_traj_interpolated[2, num_frames_interpolated - 1]
                  ],
                  [
                      P_f6_start[0], P_f6_start[1], P_f6_start[2],
                      P_f6_traj_interpolated[0, num_frames_interpolated - 1],
                      P_f6_traj_interpolated[1, num_frames_interpolated - 1],
                      P_f6_traj_interpolated[2, num_frames_interpolated - 1]
                  ]]
...
ax.scatter(P_f5_start[0],
           P_f5_start[1],
           P_f5_start[2],
           color='pink',
           label='Follower2_f1 Start',
           s=100)
ax.text(P_f5_start[0],
        P_f5_start[1],
        P_f5_start[2],
        f'({P_f5_start[0]:.2f}, {P_f5_start[1]:.2f}, {P_f5_start[2]:.2f})',
        color='black')

ax.scatter(P_f6_start[0],
           P_f6_start[1],
           P_f6_start[2],
           color='purple',
           label='Follower2_f1 Start',
           s=100)
ax.text(P_f6_start[0],
        P_f6_start[1],
        P_f6_start[2],
        f'({P_f6_start[0]:.2f}, {P_f6_start[1]:.2f}, {P_f6_start[2]:.2f})',
        color='black')
...
def update_smooth(frame):
    ...
    f2_f1_line.set_data(P_f5_traj_interpolated[0, :frame],
                        P_f5_traj_interpolated[1, :frame])
    f2_f1_line.set_3d_properties(P_f5_traj_interpolated[2, :frame])

    f2_f2_line.set_data(P_f6_traj_interpolated[0, :frame],
                        P_f6_traj_interpolated[1, :frame])
    f2_f2_line.set_3d_properties(P_f6_traj_interpolated[2, :frame])
    ...
    if frame < num_frames_interpolated - 1:
        ...
        # 连接leader和follower5的橙色虚线
        if not hasattr(update_smooth, 'line_f5'):
            update_smooth.line_f5, = ax.plot([], [], [],
                                             color='orange',
                                             linestyle='--')
        update_smooth.line_f5.set_data([
            P_l_traj_interpolated[0, frame], P_f5_traj_interpolated[0, frame]
        ], [P_l_traj_interpolated[1, frame], P_f5_traj_interpolated[1, frame]])
        update_smooth.line_f5.set_3d_properties([
            P_l_traj_interpolated[2, frame], P_f5_traj_interpolated[2, frame]
        ])

        # 连接leader和follower6的橙色虚线
        if not hasattr(update_smooth, 'line_f6'):
            update_smooth.line_f6, = ax.plot([], [], [],
                                             color='orange',
                                             linestyle='--')
        update_smooth.line_f6.set_data([
            P_l_traj_interpolated[0, frame], P_f6_traj_interpolated[0, frame]
        ], [P_l_traj_interpolated[1, frame], P_f6_traj_interpolated[1, frame]])
        update_smooth.line_f6.set_3d_properties([
            P_l_traj_interpolated[2, frame], P_f6_traj_interpolated[2, frame]
        ])
    ...
    if frame == num_frames_interpolated - 1:
        ...
        ax.text(P_f6_end[0],
                P_f6_end[1],
                P_f6_end[2],
                f'({P_f6_end[0]:.2f}, {P_f6_end[1]:.2f}, {P_f6_end[2]:.2f})',
                color='black')
        # plot l->f1->f2
        ...
        # plot l->f4->f5
        ax.plot([p_l_end[0], P_f5_end[0]], [p_l_end[1], P_f5_end[1]],
                [p_l_end[2], P_f5_end[2]],
                color='black')
        ax.plot([p_l_end[0], P_f6_end[0]], [p_l_end[1], P_f6_end[1]],
                [p_l_end[2], P_f6_end[2]],
                color='black')
        ...
        ax.plot([P_f3_end[0], P_f4_end[0]], [P_f3_end[1], P_f4_end[1]],
                ...
                color='black')
        ax.plot([P_f4_end[0], P_f5_end[0]], [P_f4_end[1], P_f5_end[1]],
                [P_f4_end[2], P_f5_end[2]],
                color='black')
        ax.plot([P_f5_end[0], P_f6_end[0]], [P_f5_end[1], P_f6_end[1]],
                [P_f5_end[2], P_f6_end[2]],
                color='black')
        ax.plot([P_f6_end[0], P_f1_end[0]], [P_f6_end[1], P_f1_end[1]],
                [P_f6_end[2], P_f1_end[2]],
                color='black')
